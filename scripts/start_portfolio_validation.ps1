...
if (-not $Confirm) {
    throw "Portfolio validation start requires -Confirm."
}
...
$seedPath = Join-Path $serverRoot "scripts\seed_portfolio_validation.py"
$runtimeRoot = Join-Path $ProjectRoot ".local\portfolio_validation"
$databasePath = Join-Path $runtimeRoot "portfolio_validation.db"
...
if (-not (Test-Path -LiteralPath $seedPath -PathType Leaf)) {
    throw "Portfolio validation seed script was not found: $seedPath"
}
...
if (Test-Path -LiteralPath $manifestPath -PathType Leaf) {
    ...
    if ($running.Count -gt 0) {
        throw "Portfolio validation processes are already running: $($running -join ', ')"
    }
}
...
try {
    ...
    try {
        ...
        & $PythonCommand `
            "scripts\seed_portfolio_validation.py" `
            --database-url $databaseUrl `
            --expected-demo-root $runtimeRoot `
            --confirm

        if ($LASTEXITCODE -ne 0) {
            throw "Portfolio validation seed failed."
        }
    }
    finally {
        Pop-Location
...
Write-Host ""
Write-Host "============================================================"
Write-Host "PORTFOLIO VALIDATION STARTED"
Write-Host "============================================================"
...
Write-Host ".\scripts\stop_portfolio_validation.ps1"
