...
if (-not $Confirm) {
    throw "Public demo start requires -Confirm."
}
...
$seedPath = Join-Path $serverRoot "scripts\seed_public_portfolio_demo.py"
$runtimeRoot = Join-Path $ProjectRoot ".local\public_demo"
$databasePath = Join-Path $runtimeRoot "public_demo.db"
...
if (-not (Test-Path -LiteralPath $seedPath -PathType Leaf)) {
    throw "Public demo seed script was not found: $seedPath"
}
...
if (Test-Path -LiteralPath $manifestPath -PathType Leaf) {
    ...
    if ($running.Count -gt 0) {
        throw "Public demo processes are already running: $($running -join ', ')"
    }
}
...
try {
    ...
    try {
        ...
        & $PythonCommand `
            "scripts\seed_public_portfolio_demo.py" `
            --database-url $databaseUrl `
            --expected-demo-root $runtimeRoot `
            --confirm

        if ($LASTEXITCODE -ne 0) {
            throw "Public demo seed failed."
        }
    }
    finally {
        Pop-Location
...
Write-Host ""
Write-Host "============================================================"
Write-Host "PUBLIC DEMO STARTED"
Write-Host "============================================================"
...
Write-Host ".\scripts\stop_public_demo.ps1"
