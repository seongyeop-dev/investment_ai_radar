...
def main() -> int:
    ...
    if not args.confirm:
        raise SystemExit("Recording session confirmation is required.")

    project_root = Path(__file__).resolve().parents[2]
    default_validation_root = (project_root / ".local" / "recording_session").resolve()
    expected_database = (default_validation_root / "recording_session.db").resolve()
    ...
    expected_suffix = Path(".local") / "recording_session" / "recording_session.db"
