...
- 최종 전체 회귀에서 Backend `625/625`, UI `43/43`, E2E `11/11`, Build `18/18 routes` PASS

## 핵심 기능

- 분석 종목 등록·수정·상세 분석
- 매수·매도 거래 기록과 보유 상태 계산
- 공식 출처 등록·수정·자동 구독 관리
- 애널리스트·전문가 공개 참고자료 등록·보관
- 자동 발견 후보 검토·승격·제외
- 중요 정보 URL 등록 전 확인과 최종 등록
- 공식 공시 등록과 종목·통합 사건 연결
- 경제·기업 일정 등록과 다음 확인 일정 연결
- 변경 기반 브리핑 Preview·Final 생성
- 사용자 위험 기준 자동 제안과 규칙 기반 관리 방향
- 시장 브리핑·알림 설정
- 출처·검증·Provider·DB·자동화 상태 확인
...
자세한 검증 범위와 실행하지 않은 운영 검증은 [05. Validation](docs/05_validation.md)에 구분해 기록했습니다.
...
13개의 편집 시연 영상으로 주요 등록·검토·연결·브리핑·설정 흐름을 정리했습니다.

전체 영상과 이미지 목록은 [10. Portfolio Media](docs/10_portfolio_media.md)와 [시연 가이드](docs/demo/README.md)에서 확인할 수 있습니다.
...
| 03 | [Features](docs/03_features.md) | 실제 구현 기능 |
| 04 | [Data Flow](docs/04_data_flow.md) | 등록·검토·사건 연결·브리핑 흐름 |
| 05 | [Validation](docs/05_validation.md) | 자동 회귀, E2E와 데이터 안전성 |
...
| 10 | [Portfolio Media](docs/10_portfolio_media.md) | 대표 이미지와 기능 시연 영상 |
| 시연 | [Demo Guide](docs/demo/README.md) | 공개 시연 순서와 촬영 기준 |

## 완료 상태

기능 구현, 포트폴리오 미디어 정리, 최종 QA, 개별 GitHub `main` 정리와 Portfolio Hub 연결까지 완료했습니다.
